...
let handler = async (m, { conn, command, usedPrefix }) => {
  // لو الجروب مفيش فيه محتوى +18، نقوله يلا باي
  if (!global.db.data.chats[m.chat].nsfw) throw `🚫 الجروب ده مفيهوش محتوى +18\n\nلو عايز تفعله، اكتب \n*${usedPrefix}enable* nsfw`

  // لو انت تحت 15 سنة، هنقولك برضو تروح تسجل في مدرسة تانية
  let user = global.db.data.users[m.sender].age
  if (user < 15) throw m.reply(`😐 انت صغير على المحتوى ده! ارجع بعدين لما تكون عندك 15 سنة على الأقل`)

  // هنعمل رد فعل زي "الوجه المنفجر" كده
  m.react('🥵') 

  switch (command) {
    case "pack":
      // هنختار صورة عشوائية من مجموعة "pack"
      let url = await pack[Math.floor(Math.random() * pack.length)]
      conn.sendFile(m.chat, url, '', '_🥵 Pack 🥵_', m)
      break

    case "pack2":
      // هنختار صورة عشوائية من مجموعة "packgirl"
      let url2 = await packgirl[Math.floor(Math.random() * packgirl.length)]
      await conn.sendFile(m.chat, url2, 'error.jpg', `_🥵 Pack 2 🥵_`, m)
      break

    case "pack3":
      // هنختار صورة عشوائية من مجموعة "packmen"
    ...
    case "videoxxx":
    case "vídeoxxx":
      // هنختار فيديو عشوائي من مجموعة "videosxxxc"
      let url4 = await videosxxxc[Math.floor(Math.random() * videosxxxc.length)]
      await conn.sendFile(m.chat, url4, 'error.mp4', `*استمتع بالفيديو 🥵*`, m)
      break
    ...
    case "pornolesbianav":
    case "pornolesv":
      // هنختار فيديو عشوائي من مجموعة "videosxxxc2"
      let url5 = await videosxxxc2[Math.floor(Math.random() * videosxxxc2.length)]
      await conn.sendFile(m.chat, url5, 'error.mp4', `*استمتع بالفيديو 🥵*`, m)
      break    
  }
}

handler.tags = ['nsfw']
handler.command = /^(pack|pack2|بورن|سيكوسيكو|pack3|videoxxx|vídeoxxx|videoxxxlesbi|videolesbixxx|pornolesbivid|pornolesbianavid|pornolesbiv|pornolesbianav|pornolesv)$/i
